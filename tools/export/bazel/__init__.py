...
class Bazel(Exporter):
    """Generic bazel template that mimics the behavior of the python build
    system
    """
    ...
    def generate(self):
        ...
        for flag in ctx['asm_flags']:
            if flag.startswith('-I'):
                new_asm_flags.append(
                    "-I{}/{}".format(ctx['vpath'][0], flag[2:]))
            elif flag.startswith('--preinclude='):
                new_asm_flags.append(
                    "--preinclude={}/{}".format(ctx['vpath'][0], flag[13:]))
            else:
                new_asm_flags.append(flag)
        ctx['asm_flags'] = new_asm_flags

        try:
            self.gen_file('bazel/WORKSPACE.tmpl', ctx, 'WORKSPACE')
            self.gen_file('bazel/BUILD.tmpl', ctx, 'BUILD')
            break
    ...
    def clean(_):
        remove("WORKSPACE")
        remove("BUILD")
        bazel_generated = glob('bazel-*')
        if bazel_generated:
            shutil.rmtree(bazel_generated)
